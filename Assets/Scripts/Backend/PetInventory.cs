...
namespace Lance
{
    ...
    public class PetEquipmentInst
    {
        ...
        public PetEquipmentInst(string id, int level, int count, bool isEquipped, int reporge, int failedReforge, int stackedFailedReforge, int tryReforge, EquipmentOptionStatInfo optionStat)
        {
            mId = id;
            mLevel = level;
            mCount = count;
        ...
        {
            return mData.maxLevel + mReforgeStep * Lance.GameData.PetEquipmentCommonData.reforgeAddMaxLevel;
        }
        ...
        {
            return DataUtil.GetPetEquipmentUpgradeRequireStone(mData, mLevel, upgradeCount);
        }

        public ObscuredDouble GetReforgeRequireStone()
        {
            return DataUtil.GetPetEquipmentReforgeRequireStone(mData.grade, mReforgeStep);
        }

        public ObscuredInt GetReforgeRequireElementalStone()
        {
            return DataUtil.GetPetEquipmentReforgeRequireElementalStone(mData.grade, mReforgeStep);
        }
    ...
    // 장비 관리
    public class PetInventory : AccountBase
    {
        ObscuredInt mStackedCombineCount;

        Dictionary<ObscuredString, PetEquipmentInst> mEquipmentItems = new();
        protected override void SetServerDataToLocal(JsonData gameDataJson)
        {
            ...
            for (int i = 0; i < equipmentsData.Count; i++)
            {
                ...
                if (levelTemp > 0)
                {
                    ...
                    mEquipmentItems.Add(id, new PetEquipmentInst(id, levelTemp, countTemp, isEquipped,
        ...
        {
            Dictionary<string, PetEquipmentInst> saveEquipmentItems = new Dictionary<string, PetEquipmentInst>();
        ...
        public PetEquipmentInst GetEquipItem(string id)
        {
            return mEquipmentItems.TryGet(id);
        }

        public PetEquipmentInst GetEquipItem(Grade grade, SubGrade subGrade)
        {
        ...
        public PetEquipmentInst GetEquippedItem()
        {
        ...
        public IEnumerable<PetEquipmentInst> GetEquipItems()
        {
        ...
        public void AddItem(string id, int count)
        {
            ...
            {
                PetEquipmentInst equipInst = mEquipmentItems.TryGet(id);
                if (equipInst != null)
                {
            ...
            else
            {
                ...
                mEquipmentItems.Add(id, new PetEquipmentInst(id, 1, count, false, 0, 0, 0, 0, optionStat));
            }

            SetIsChangedData(true);
        }

        public PetEquipmentInst RemoveItem(string id)
        {
            if (mEquipmentItems.ContainsKey(id))
            {
                var equipInst = mEquipmentItems.TryGet(id);

                mEquipmentItems.Remove(id);
        ...
        {
            PetEquipmentInst bestEquipment = FindBestEquipment();
            ...
            PetEquipmentInst FindBestEquipment()
            {
                PetEquipmentInst bestEquipment = null;
                double bestPowerLevel = 0;

                foreach (PetEquipmentInst equipment in mEquipmentItems.Values)
                {
                    double powerLevel = PowerLevelCalculator.CalcPetEquipmentPowerLevel(equipment.GetId());
        ...
        PetEquipmentInst FindEquippedItem()
        {
            PetEquipmentInst equippedItem = null;
        ...
        public bool EquipItem(string id)
        {
            ...
            // 장비 착용
            PetEquipmentInst equipItem = GetEquipItem(id);
        ...
        public (string id, int combineCount) CombineItem(string id)
        {
            ...
            // 다음 등급 장비 데이터를 확인
            // 데이터가 없다면 뭔가 잘 못된 것
            PetEquipmentData nextData = DataUtil.GetNextGradePetEquipmentData(data.type, data.grade, data.subGrade);
            if (nextData == null)
                return (string.Empty, 0);
            ...
            for (int i = 0; i < (int)Grade.Count; ++i)
            {
                for (int j = 0; j < (int)SubGrade.Four_Star; ++j)
                {
                    ...
                    if (inst != null)
                    {
                        ...
                        if (result.id.IsValid())
                        {
                            PetEquipmentInst nextInst = GetEquipItem(result.id);
                            if (nextInst != null)
                            {
                            ...
                            //var nextInst = GetEquipItem(result.id);
                            //if (nextInst != null)
                            //{
                            //    int combineCount = nextInst.GetCombineCount();
        ...
        public double GatherStatValues(StatType type)
        {
            ...
            foreach (PetEquipmentInst item in mEquipmentItems.Values)
            {
                if (item.IsEquipped())
                {
