...
namespace Lance
{
    class Inventory_EquipmentItemUI : MonoBehaviour
    {
        string mId;
        EquipmentData mData;
        ...
        {
            mId = id;
            mData = DataUtil.GetEquipmentData(mId);
        ...
        void RefreshRedDot()
        {
            mRedDotObj.SetActive(mIsIgnoreRedDot ? false : RedDotUtil.IsActiveEquipmentRedDot(mId));
        }
    ...
    class SubGradeManager
    {
        GameObject mParent;

        Image[] mImageSubgrades;

        public void Init(GameObject parent)
        {
            mParent = parent;

            mImageSubgrades = mParent.GetComponentsInChildren<Image>(true);
        }

        public void SetSubGrade(SubGrade subGrade)
        {
            SetSubGrade((UISubGrade)subGrade);
        }

        public void SetSubGrade(UISubGrade subGrade)
        {
            for(int i = 0; i < (int)UISubGrade.Count; ++i)
            {
                if (i <= (int)UISubGrade.Five_Star)
                {
                    bool active = i <= (int)subGrade;

                    mImageSubgrades[i].gameObject.SetActive(active);

                    mImageSubgrades[i].sprite = Lance.Atlas.GetItemSlotUISprite("Icon_RarityLevel_Star");
                }
                else
                {
                    bool active = i <= (int)subGrade;
                    if(active)
                    {
                        int index = i % (int)UISubGrade.Six_Star;

                        mImageSubgrades[index].sprite = Lance.Atlas.GetItemSlotUISprite("Icon_RarityLevel_Star_Red");
                    }
                }
            }
        }
        ...
        {
            mParent.gameObject.SetActive(active);
        }
    }
}