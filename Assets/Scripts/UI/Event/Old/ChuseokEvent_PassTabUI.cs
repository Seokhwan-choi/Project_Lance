...
namespace Lance
{
    class ChuseokEvent_PassTabUI : ChuseokTabUI
    {
        string mPassId;
        ...
        {
            foreach (ChuseokEventPassItemUI passItem in GetPassItems())
            {
        ...
        IEnumerable<ChuseokEventPassItemUI> GetPassItems()
        {
            foreach (var rect in mDynamicScrollView.GetContainters())
                yield return rect.GetComponent<ChuseokEventPassItemUI>();
        }
