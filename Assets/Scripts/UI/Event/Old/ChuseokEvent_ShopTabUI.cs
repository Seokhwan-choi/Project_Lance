...
namespace Lance
{
    class ChuseokEvent_ShopTabUI : ChuseokTabUI
    {
        List<ChuseokEventShopItemUI> mEventItemList;
        public override void Init(ChuseokTabUIManager parent, string eventId, ChuseokTab tab)
        {
            ...
            mEventItemList = new List<ChuseokEventShopItemUI>();

            foreach (var data in Lance.GameData.EventShopData.Values.Where(x => x.eventId == eventId))
            {
                var eventItemObj = Util.InstantiateUI("ChuseokEventShopItemUI", contentsObj.transform);

                var eventItemUI = eventItemObj.GetOrAddComponent<ChuseokEventShopItemUI>();
    ...
    class ChuseokEventShopItemUI : MonoBehaviour
    {
        string mEventId;
        string mId;
        ChuseokEvent_ShopTabUI mParent;
        TextMeshProUGUI mTextPurchaseCount;
        GameObject mModalObj;
        public void Init(ChuseokEvent_ShopTabUI parent, string eventId, string id)
        {
            mParent = parent;
            mEventId = eventId;
            mId = id;
