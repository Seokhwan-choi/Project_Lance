...
namespace Lance
{
    class HalloweenEvent_ShopTabUI : HalloweenTabUI
    {
        List<HalloweenEventShopItemUI> mEventItemList;
        public override void Init(HalloweenTabUIManager parent, string eventId, HalloweenTab tab)
        {
            ...
            mEventItemList = new List<HalloweenEventShopItemUI>();

            foreach (var data in Lance.GameData.EventShopData.Values.Where(x => x.eventId == eventId))
            {
                var eventItemObj = Util.InstantiateUI("HalloweenEventShopItemUI", contentsObj.transform);

                var eventItemUI = eventItemObj.GetOrAddComponent<HalloweenEventShopItemUI>();
    ...
    class HalloweenEventShopItemUI : MonoBehaviour
    {
        string mEventId;
        string mId;
        HalloweenEvent_ShopTabUI mParent;
        TextMeshProUGUI mTextPurchaseCount;
        GameObject mModalObj;
        public void Init(HalloweenEvent_ShopTabUI parent, string eventId, string id)
        {
            mParent = parent;
            mEventId = eventId;
            mId = id;
