...
namespace Lance
{
    class SnakeNewYearEvent_PassTabUI : SnakeNewYearTabUI
    {
        string mPassId;
        ...
        {
            foreach (SnakeNewYearEventPassItemUI passItem in GetPassItems())
            {
        ...
        IEnumerable<SnakeNewYearEventPassItemUI> GetPassItems()
        {
            foreach (var rect in mDynamicScrollView.GetContainters())
                yield return rect.GetComponent<SnakeNewYearEventPassItemUI>();
        }
