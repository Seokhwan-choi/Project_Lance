...
namespace Lance
{
    class Popup_ConfirmPurchaseUI4 : PopupBase
    {
        int mPurchaseCount;
        EventShopData mData;
        TextMeshProUGUI mTextPurchaseCount;
        TextMeshProUGUI mTextTotalPrice;
        public void Init(EventShopData data, Action onConfirm)
        {
            SetUpCloseAction();
            ...
            {
                if (Lance.GameManager.PurchaseEventItem(mData.eventId, mData.id, mPurchaseCount))
                {
                    onConfirm?.Invoke();
            ...
            imageCurrency.sprite = Lance.Atlas.GetItemSlotUISprite($"Currency_{mData.eventId}");
            ...
            buttonMax.SetButtonAction(() => SetPurchaseCount(mData.purchaseCount));
        ...
        {
            int maxCount = Math.Min(mData.purchaseCount, Lance.Account.Event.GetRemainPurchaseCount(mData.eventId, mData.id));
