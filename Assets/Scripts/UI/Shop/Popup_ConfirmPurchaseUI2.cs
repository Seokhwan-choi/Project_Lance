...
namespace Lance
{
    class Popup_ConfirmPurchaseUI2 : PopupBase
    {
        int mPurchaseCount;
        NormalShopData mData;
        TextMeshProUGUI mTextPurchaseCount;
        TextMeshProUGUI mTextTotalPrice;
        public void Init(NormalShopData data, Action onConfirm)
        {
            SetUpCloseAction();
            ...
            {
                if (Lance.GameManager.PurchaseNormalItem(data.id, mPurchaseCount) )
                {
                    onConfirm?.Invoke();
        ...
        {
            int maxCount = Math.Min(mData.purchaseDailyCount, Lance.Account.NormalShop.GetRemainPurchaseCount(mData.id));
