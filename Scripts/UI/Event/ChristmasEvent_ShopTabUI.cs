...
namespace Lance
{
    class ChristmasEvent_ShopTabUI : ChristmasTabUI
    {
        List<ChristmasEventShopItemUI> mEventItemList;
        public override void Init(ChristmasTabUIManager parent, string eventId, ChristmasTab tab)
        {
            ...
            mEventItemList = new List<ChristmasEventShopItemUI>();

            foreach (var data in Lance.GameData.EventShopData.Values.Where(x => x.eventId == eventId))
            {
                var eventItemObj = Util.InstantiateUI("ChristmasEventShopItemUI", contentsObj.transform);

                var eventItemUI = eventItemObj.GetOrAddComponent<ChristmasEventShopItemUI>();
    ...
    class ChristmasEventShopItemUI : MonoBehaviour
    {
        string mEventId;
        string mId;
        ChristmasEvent_ShopTabUI mParent;
        TextMeshProUGUI mTextPurchaseCount;
        GameObject mModalObj;
        public void Init(ChristmasEvent_ShopTabUI parent, string eventId, string id)
        {
            mParent = parent;
            mEventId = eventId;
            mId = id;
