...
namespace Lance
{
    class SummerEvent_ShopTabUI : SummerTabUI
    {
        List<NewEventShopItemUI> mEventItemList;
        public override void Init(SummerTabUIManager parent, string eventId, SummerTab tab)
        {
            ...
            mEventItemList = new List<NewEventShopItemUI>();

            foreach (var data in Lance.GameData.EventShopData.Values.Where(x => x.eventId == eventId))
            {
                var eventItemObj = Util.InstantiateUI("EventShopItemUI", contentsObj.transform);

                var eventItemUI = eventItemObj.GetOrAddComponent<NewEventShopItemUI>();
    ...
    class NewEventShopItemUI : MonoBehaviour
    {
        string mEventId;
        string mId;
        SummerEvent_ShopTabUI mParent;
        TextMeshProUGUI mTextPurchaseCount;
        GameObject mModalObj;
        public void Init(SummerEvent_ShopTabUI parent, string eventId, string id)
        {
            mParent = parent;
            mEventId = eventId;
            mId = id;
