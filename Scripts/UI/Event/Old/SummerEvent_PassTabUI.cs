...
namespace Lance
{
    class SummerEvent_PassTabUI : SummerTabUI
    {
        string mPassId;
        ...
        {
            foreach (EventPassItemUI passItem in GetPassItems())
            {
        ...
        IEnumerable<EventPassItemUI> GetPassItems()
        {
            foreach (var rect in mDynamicScrollView.GetContainters())
                yield return rect.GetComponent<EventPassItemUI>();
        }
