...
namespace Lance
{
    class HalloweenEvent_PassTabUI : HalloweenTabUI
    {
        string mPassId;
        ...
        {
            foreach (HalloweenEventPassItemUI passItem in GetPassItems())
            {
        ...
        IEnumerable<HalloweenEventPassItemUI> GetPassItems()
        {
            foreach (var rect in mDynamicScrollView.GetContainters())
                yield return rect.GetComponent<HalloweenEventPassItemUI>();
        }
