...
namespace Lance
{
    class Popup_AccessoryProbUI : PopupBase
    {
        ...
        void ChangeLevel(bool isDown)
        {
            ...
            changeLevel = Mathf.Clamp(changeLevel, 1, Lance.GameData.ShopCommonData.accessorySpawnMaxLevel);
        ...
        void Refresh()
        {
            if (mProbData != null)
            {
                ...
                for (int i = (int)Grade.SSR; i >= 0; --i)
                {
                    ...
                    if (gradeProb > 0)
                    {
                        float[] subGradeProbs = DataUtil.GetAccessorySubgradeProbs(grade, mProbData.level);
