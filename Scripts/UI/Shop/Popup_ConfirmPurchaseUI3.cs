...
namespace Lance
{
    class Popup_ConfirmPurchaseUI3 : PopupBase
    {
        int mPurchaseCount;
        MileageShopData mData;
        TextMeshProUGUI mTextPurchaseCount;
        TextMeshProUGUI mTextTotalPrice;
        public void Init(MileageShopData data, Action onConfirm)
        {
            SetUpCloseAction();
            ...
            {
                if ( Lance.GameManager.PurchaseMileageItem(data.id, mPurchaseCount) )
                {
                    onConfirm?.Invoke();
            ...
            imageCurrency.sprite = Lance.Atlas.GetItemSlotUISprite($"Currency_Mileage");
        ...
        {
            int maxCount = Math.Min(mData.purchaseDailyCount, Lance.Account.MileageShop.GetRemainPurchaseCount(mData.id));
