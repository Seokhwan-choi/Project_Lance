...
namespace Lance
{
    ...
    public class AccountBase : BackendBase
    {
        ...
        public virtual string GetTableName() { return string.Empty; }  // 자식 객체가 설정한 테이블 이름 가져오는 함수
        public virtual string GetColumnName() { return string.Empty; } // 자식 객체가 설정한 컬럼 이름 가져오는 함수
        public virtual Param GetParam() { return new Param(); } // 자식 객체가 설정한 업데이트에 사용될 Param을 가져오는 함수
        protected virtual void InitializeData() { } // 자식 객체가 설정한 데이터 초기화를 해주는 함수
        protected virtual void SetServerDataToLocal(JsonData gameDataJson) { } // 자식 객체가 설정한 불러오기 함수 호출 이후 각 테이블에 맞게 파싱하는 함수
